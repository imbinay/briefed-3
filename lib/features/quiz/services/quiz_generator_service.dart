...
class QuizGeneratorService {
  ...
  static Future<List<QuizQuestion>> generate(
    ...
    if (!forceRefresh) {
      // L1: local SharedPreferences cache (instant)
      final cached = await QuizCacheService.load(cacheCategory);
      if (cached != null && cached.isNotEmpty) {
        dev.log('Serving local cached quiz (${cached.length} Qs)', name: _tag);
        return cached;
      }

      // L2: Firestore server cache (pre-generated by Cloud Function, shared across users)
      ...
      if (serverQuestions != null && serverQuestions.isNotEmpty) {
        dev.log('Serving server quiz (${serverQuestions.length} Qs)', name: _tag);
        await QuizCacheService.save(cacheCategory, serverQuestions);
        return serverQuestions;
      }
    }

    // L3: on-device generation with Groq/Gemini
